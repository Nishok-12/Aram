...
#Aram IDE GUI_Version_3 (Supports All Highliting and Error fully functional but Input is again in error (i.e.Not working))
...
class ARAMWindow:
    ...
    def request_input(self, prompt=""):
        ...
        # Show prompt immediately in the result_text BEFORE placing the Entry
        if prompt:
    ...
    def _setup_gui(self):
        ...
        # ---------- Add Context Menu ----------
        self.context_menu = Menu(self.root, tearoff=0, font=("Arial", 10))  # smaller font

        def add_menu_item(menu, text, shortcut=None, command=None):
            if shortcut:
                # Add item with label + command
                menu.add_command(label=text, command=command)
                # Add fake right-aligned shortcut as disabled
    ...
    def run_code(self):
        ...
        self.result_text.insert(
            tk.END,
            f"\n[முடிந்தது] {filename} குறியீடு {elapsed:.3f} வினாடிகளில் நிறைவுற்றது\n",
            "done_msg"
        )
