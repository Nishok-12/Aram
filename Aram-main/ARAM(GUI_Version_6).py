...
#Aram IDE GUI_Version_6 (Auto complete glitch version with corrected Autocomplete and auto-conversion)
...
class AutocompletePopup:
    def __init__(self, master, code_text, suggestions):
        self.master = master
    ...
    def show(self):
        ...
        pos = self.code_text.bbox(tk.INSERT)
        if not pos:
            return
        x, y, _, h = pos
        x += self.code_text.winfo_rootx()
        y += self.code_text.winfo_rooty() + h

        self.popup = tk.Toplevel(self.master)
        self.popup.wm_overrideredirect(True)
        self.popup.wm_geometry(f"+{x}+{y}")
        self.listbox = tk.Listbox(self.popup, height=min(len(self.suggestions), 10),
        ...
        self.listbox.bind("<Tab>", self.select)
        self.listbox.bind("<Double-Button-1>", self.select)
        self.listbox.bind("<Escape>", lambda e: self.hide())
        self.code_text.bind("<Down>", self.move_down)
        self.code_text.bind("<Up>", self.move_up)
    ...
    def hide(self):
        if self.popup:
            ...
            self.popup = None
            self.listbox = None
            # Rebind keys after hiding the popup
            self.code_text.bind("<Down>", self.master.on_key_release)
            self.code_text.bind("<Up>", self.master.on_key_release)
    ...
    def select(self, event):
        ...
        # Manually find the start of the current word
        start_index = self.code_text.index(tk.INSERT)
        while start_index != "1.0" and self.code_text.get(f"{start_index}-1c") not in " \n\t()[]{}.,;+-*/=<>!":
            start_index = f"{start_index}-1c"
        
        self.code_text.delete(start_index, tk.INSERT)
        self.code_text.insert(start_index, selected_text)
    ...
    def move_down(self, event):
        ...
        if not current_sel:
            ...
            next_index = current_sel[0] + 1
            if next_index < self.listbox.size():
                self.listbox.selection_clear(current_sel[0])
...
class ARAMWindow:
    ...
    def request_input(self, prompt=""):
        ...
        # Wait on the event, allowing the GUI to remain responsive
        input_ready.wait()
    ...
    def _setup_gui(self):
        ...
        def update_editor(event=None):
            ...
            # Reset tags for both text boxes
            ...
            self.code_text.tag_config("comment", foreground="grey") 
            ...
            # Find and highlight comments in the English pane
            for match in re.finditer(r'^\s*#.*$', english_code, re.MULTILINE):
                start_index = f"1.0+{match.start()}c"
                end_index = f"1.0+{match.end()}c"
                self.english_text.config(state="normal")
                self.english_text.tag_add("comment", start_index, end_index)
                self.english_text.config(state="disabled")
            self.english_text.tag_config("comment", foreground="grey") 
    ...
    def get_suggestions(self):
        code = self.code_text.get("1.0", tk.END)
        # Match Tamil or English words and underscores.
        words = re.findall(r'[\u0B80-\u0BFFa-zA-Z_]+', code)
        
        tamil_keywords = set(TAMIL_TO_ENGLISH.keys())
        english_keywords = set(TAMIL_TO_ENGLISH.values())
        
        # Combine all suggestions into a single set
        suggestions = set(tamil_keywords)
        
        # Add user-defined words if they are not already keywords
        for word in words:
            if word not in tamil_keywords and word not in english_keywords:
                suggestions.add(word)
                
        # Add common syntax elements
        suggestions.add("()")
        suggestions.add(":")
        suggestions.add("()")
        suggestions.add("[]")
    ...
    def on_key_release(self, event):
        # Check for English-to-Tamil auto-conversion first
        if self._auto_convert(event):
            return

        # Hide popup on space, enter, or punctuation
        if event.char in " \n\t()[]{}.,;+-*/=<>!":
            if self.autocomplete_popup:
                self.autocomplete_popup.hide()
            return

        # Don't show popup for navigation keys
        if event.keysym in ("Down", "Up", "Right", "Left", "Return", "Tab"):
            return
        ...
        matching_suggestions = [s for s in suggestions if s.startswith(current_word)]

        if matching_suggestions:
            if self.autocomplete_popup:
                self.autocomplete_popup.hide()
            self.autocomplete_popup = AutocompletePopup(self.root, self.code_text, matching_suggestions)
        else:
            if self.autocomplete_popup:
                self.autocomplete_popup.hide()

    def _get_current_word(self):
        """Manually determines the word before the cursor."""
        try:
            start_index = self.code_text.index(tk.INSERT)
            current_line = self.code_text.get(f"{start_index} linestart", start_index)
            # Find the last word on the line
            words_on_line = re.findall(r'[\u0B80-\u0BFFa-zA-Z_]+', current_line)
            if words_on_line:
                return words_on_line[-1]
            return ""
        except Exception:
            return ""

    def _auto_convert(self, event):
        """Converts typed English keywords to Tamil."""
        # Only check on completion characters like space, tab, etc.
        if event.char not in " \n\t()[]{}.,;+-*/=<>!":
            return False

        # Get the word just before the cursor
        cursor_index = self.code_text.index(tk.INSERT)
        word_end_index = self.code_text.index(f"{cursor_index}-1c")
        
        # This is a robust way to find the word's start
        word_start_index = word_end_index
        while True:
            if word_start_index == '1.0':
                break
            char = self.code_text.get(f"{word_start_index}-1c")
            if char in " \n\t()[]{}.,;+-*/=<>!":
                break
            word_start_index = f"{word_start_index}-1c"

        word_to_check = self.code_text.get(word_start_index, word_end_index).strip()
        
        reverse_map = {v: k for k, v in TAMIL_TO_ENGLISH.items()}
        
        if word_to_check in reverse_map:
            tamil_word = reverse_map[word_to_check]
            self.code_text.delete(word_start_index, word_end_index)
            self.code_text.insert(word_start_index, tamil_word)
            self.code_text.event_generate("<<Change>>")
            return True
        return False
        ...
        def check_queue():
            try:
                while True:
                    ...
                    self.result_text.tag_config("done_msg", foreground="blue") 
