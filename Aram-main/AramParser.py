##########################################
# Parser for Aram Programming Language
##########################################

import error
from sys import exit

STRUCTURE = ['IF', 'FOR', 'WHILE', 'FUNC', 'TRY']
IO = ['PRINT', 'INPUT']
...
class TupleIndexNode:
    def __init__(self, tuple_element, index):
        self.tuple_element = tuple_element
        self.index = index
    # In AramParser.py (simplified example)
    def parse_primary_expression(self):
        node = None
        if self.current_token.type == 'VARIABLE':
            node = Variable(self.current_token.value)
            self.eat('VARIABLE')
        # ... (other code for numbers, strings, etc.) ...
        
        # Check for list or tuple indexing after the variable
        while self.current_token.type == 'LBRACKET':
            self.eat('LBRACKET')
            index_node = self.parse_expression()
            self.eat('RBRACKET')
            
            # This is where you decide if it's a list or tuple.
            # Since the Interpreter handles the type check at runtime,
            # you can create a generic index node or a specific one.
            # A specific node is clearer.
            node = TupleIndexNode(node, index_node)
...
class TryCatchNode:
    def __init__(self, try_block, except_var=None, except_block=None, finally_block=None):
        self.try_block = try_block
        self.except_var = except_var
        self.except_block = except_block
        self.finally_block = finally_block
    def __repr__(self):
        return f'Try({self.try_block}) Except({self.except_var}, {self.except_block}) Finally({self.finally_block})'
...
class Parser:
    ...
    def func_call_statement(self):
        ...
        self.nextToken()
        if self.currentToken.type != 'LPAREN':
            error.Error("ஒரு செய் அழைப்பிற்கு பின் '(' எதிர்பார்க்கப்பட்டது")
            self.hasErrored = True
            ...
            while self.currentToken != 'EOF' and self.currentToken.type != 'RPAREN':
                ...
                if self.currentToken.type == 'COMMA':
                    ...
                    if self.currentToken.type == 'RPAREN' or self.currentToken.type == 'COMMA':
                        error.Error("செய் அழைப்பில் ',' பின் ஒரு parameter எதிர்பார்க்கப்பட்டது")
                        self.hasErrored = True
    ...
    def structure(self):
        ...
        # grammar rule: loop -> for_loop | while_loop (avoided coding a method for loop)
            ...
            res = self.function()
            return res
        elif self.currentToken.type == 'TRY':
            res = self.try_catch_statement()
            return res
    ...
    def function(self):
        self.nextToken()
        if self.currentToken.type != 'ID':
            error.Error("'செய்' பின் செயல்பாகத்திற்கான பெயர் எதிர்பார்க்கப்பட்டது")
            self.hasErrored = True
            ...
            self.nextToken()
            if self.currentToken.type != 'LPAREN':
                error.Error("'செய்' பின் '()' எதிர்பார்க்கப்பட்டது")
                self.hasErrored = True
                ...
                if self.currentToken.type == 'RPAREN':
                    ...
                    if self.currentToken.type != 'COLON':
                        ...
                        if self.currentToken == 'EOF':
                            error.Error("'செய்()' முடிவில் 'முடி' எதிர்பார்க்கப்பட்டது")
                            self.hasErrored = True
    ...
    def for_loop(self):
        self.nextToken()
        if self.currentToken.type != 'LPAREN':
            error.Error("'சுற்று' பின் '()' எதிர்பார்க்கப்பட்டது")
            self.hasErrored = True
            ...
            if self.currentToken.type != 'COLON':
                ...
                if self.currentToken == 'EOF':
                    error.Error("'சுற்று()' முடிவில் 'முடி' எதிர்பார்க்கப்பட்டது")
                    self.hasErrored = True
    ...
    #           statement ([ELIF][LPAREN] (logical) [RPAREN] [:] statement)* #           ([ELSE] [LPAREN] (logical) [RPAREN] [:] statement)? END
    def conditional(self):
        self.nextToken()
        if self.currentToken.type != 'LPAREN':
            error.Error("'எனில்' பின் '()' எதிர்பார்க்கப்பட்டது")
            self.hasErrored = True
        ...
        if self.currentToken.type == 'END':
            ...
            while self.currentToken.type == 'ELIF' and self.currentToken != 'EOF':
                ...
                if self.currentToken.type == 'COLON':
                    ...
                    error.Error("'பிறஎனில்()' பின் ':' எதிர்பார்க்கப்பட்டது")
            if self.currentToken == 'EOF':
                error.Error("Conditional Block முடிவில் 'முடி' எதிர்பார்க்கப்பட்டது")
                self.hasErrored = True
            ...
            if self.currentToken.type == 'END':
                ...
                if self.currentToken.type == 'ELSE' and self.currentToken != 'EOF':
                    ...
                    if self.currentToken.type == 'COLON':
                        ...
                        error.Error("'பொய்எனில்' பின் ':' எதிர்பார்க்கப்பட்டது")
                    if self.currentToken == 'EOF':
                        error.Error("Conditional Block முடிவில் 'முடி' எதிர்பார்க்கப்பட்டது")
                        self.hasErrored = True
    ...
    # grammar rule: comparison -> [NOT] comparison | bitwise_or ([==|!=|<|>|<=|>=] bitwise_or)*
    def comparison(self):
        if self.currentToken != 'EOF' and self.currentToken.type == 'NOT':
            ...
            return node
        leftExp = self.bitwise_or()
        while self.currentToken != 'EOF' and self.currentToken.type in ('EQ', 'NOT_EQ', 'LESS_EQ', 'LESS', 'GREATER_EQ', 'GREATER'):
            operator = self.currentToken.type
            self.nextToken()
            rightExp = self.bitwise_or()
            leftExp = CompOpNode(leftExp, operator, rightExp)
        return leftExp

    # grammar rule: bitwise_or -> bitwise_xor ([BIT_OR] bitwise_xor)*
    def bitwise_or(self):
        left_node = self.bitwise_xor()
        while self.currentToken != 'EOF' and self.currentToken.type == 'BIT_OR':
            op_token = self.currentToken
            self.nextToken()
            right_node = self.bitwise_xor()
            left_node = BinOpNode(left_node, op_token.type, right_node)
        return left_node

    # grammar rule: bitwise_xor -> bitwise_and ([BIT_XOR] bitwise_and)*
    def bitwise_xor(self):
        left_node = self.bitwise_and()
        while self.currentToken != 'EOF' and self.currentToken.type == 'BIT_XOR':
            op_token = self.currentToken
            self.nextToken()
            right_node = self.bitwise_and()
            left_node = BinOpNode(left_node, op_token.type, right_node)
        return left_node

    # grammar rule: bitwise_and -> shift ([BIT_AND] shift)*
    def bitwise_and(self):
        left_node = self.shift()
        while self.currentToken != 'EOF' and self.currentToken.type == 'BIT_AND':
            op_token = self.currentToken
            self.nextToken()
    ...
    # grammar rule: shift -> expression ([LSHIFT|RSHIFT] expression)*
    def shift(self):
        left_node = self.expression()
        while self.currentToken != 'EOF' and self.currentToken.type in ('LSHIFT', 'RSHIFT'):
            op_token = self.currentToken
            self.nextToken()
    ...
    # grammar rule: factor -> [plus | minus | bit_not] factor | [int | float] 
    #       | [LPAREN]logical[RPAREN] | variable | string 
    #       | list | listIndex | function_call_statement
    def factor(self):
        if self.currentToken != 'EOF':
            if (self.currentToken.type in ('PLUS', 'MINUS', 'BIT_NOT')):
    ...
    def try_catch_statement(self):
        ...
        self.nextToken()
        if self.currentToken.type != 'COLON':
            error.Error("'முயற்சி' பின் ':' எதிர்பார்க்கப்பட்டது")
            self.hasErrored = True
             
        self.nextToken()
        try_block = []
        while self.currentToken != 'EOF' and self.currentToken.type not in ('EXCEPT', 'FINALLY', 'END'):
        ...
        if self.currentToken.type == 'EXCEPT':  # 'தவிர'
            self.nextToken()
            if self.currentToken.type == 'ID':
                except_var = self.currentToken.value
                self.nextToken()
            if self.currentToken.type != 'COLON':
                error.Error("'தவிர' பின் ':' எதிர்பார்க்கப்பட்டது")
                self.hasErrored = True
                 
            self.nextToken()
            except_block = []
            while self.currentToken != 'EOF' and self.currentToken.type not in ('FINALLY', 'END'):
                except_block.append(self.statement())

        if self.currentToken.type == 'FINALLY':  # 'முடிவில்'
            self.nextToken()
            if self.currentToken.type != 'COLON':
                error.Error("'முடிவில்' பின் ':' எதிர்பார்க்கப்பட்டது")
                self.hasErrored = True
        ...
        if self.currentToken.type != 'END':
            error.Error("'முயற்சி' block முடிவில் 'முடி' எதிர்பார்க்கப்பட்டது")
            self.hasErrored = True
             

        self.nextToken()
        return TryCatchNode(try_block, except_var, except_block, finally_block)
...
#########################################################
#       Grammar for Aram Programming Language
#########################################################
