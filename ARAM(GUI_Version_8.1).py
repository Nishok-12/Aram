...
# Aram IDE GUI_Version_8.1  (Adds functions like drak mode switch (except left pane and scroll bar), font size increase and decrease)
    ...
    def _build(self):
        ...
        for title, items in self.sections:
            ...
            for tamil, eng in items:
                lbl = tk.Label(frame, text=tamil, anchor="w", bg=frame_bg, fg="#4cff4c" if self.theme == "dark" else "green",
...
class ARAMWindow:
    ...
    def _auto_complete_brackets(self, event):
        ...
        if char == '(':
            # Try Tamil replacement first
            word, start, end = self._current_word_range()
            if word:
                repl = ENGLISH_TO_TAMIL.get(word)
                if repl:
                    # Replace the English word with Tamil equivalent
                    self.code_text.delete(start, end)
        ...
        # First, handle the word replacement if the key is a parenthesis
        if char == '(':
            # Assuming you have a separate function to handle word replacement
            # You need to call it here. Let's call it _replace_word()
            # This function should handle the logic to get the word, find its Tamil translation,
            # and replace it in the text.
            self._replace_word(event)
        
        # Now, check if the typed character is in our map
        if char in completion_map:
            # Get the closing character
            closing_char = completion_map[char]
            
            # Manually insert both the opening and closing characters
            self.code_text.insert(tk.INSERT, char + closing_char)
            
            # Move the cursor back one position to place it in the middle
            self.code_text.mark_set(tk.INSERT, "insert-1c")
            
            # Return "break" to stop the default behavior
            return "break"

    # Note: The _replace_word function would contain your existing logic for
    # word-to-Tamil replacement and would be called by the above function.
    ...
    def _setup_gui(self):
        ...
        buttons = [
            ...
            ("🌙/☀", self.toggle_theme, "#fff", "Theme"),
            ("அ+", self.increase_font_size, "#fff", "Increase Font"),
            ("அ-", self.decrease_font_size, "#fff", "Decrease Font"),
        ...
        for txt, cmd, color, shortcut in buttons:
            ...
            if self.current_theme == "dark":
                ...
                # Only convert white buttons to dark gray
                if color == "#fff":
        ...
        self.code_text = CodeText(editor_wrap, wrap="none", font=("Courier New",12), bg="white", 
                                  yscrollcommand=y_scroll.set, 
                                  xscrollcommand=x_scroll.set, # Link the x-scrollbar
                                  undo=True, autoseparators=True, maxundo=-1)
        y_scroll.config(command=self.code_text.yview)
        x_scroll.config(command=self.code_text.xview) # Configure the new x-scrollbar
        ...
        # Add a horizontal scrollbar for the English pane
        x_scroll_eng = tk.Scrollbar(editor_wrap_eng, orient="horizontal")
        x_scroll_eng.pack(side="bottom", fill="x")
        
        self.english_text = CodeText(editor_wrap_eng, wrap="none", font=("Courier New",12), bg="white", 
                                     yscrollcommand=y_scroll_eng.set,
                                     xscrollcommand=x_scroll_eng.set)  # Link the x-scrollbar to the English text widget
        y_scroll_eng.config(command=self.english_text.yview)
        x_scroll_eng.config(command=self.english_text.xview)  # Configure the new x-scrollbar
        ...
        self.result_text.tag_config("welcome_msg", foreground= "#0342FF", font=("Courier New", 11, "italic"))
        self.result_text.see(tk.END)
        ...
        def update_editor_and_features(event=None):
            ...
            # Find all lines with '#' and apply the 'comment' tag
            comment_pattern = r"#.*"
            for match in re.finditer(comment_pattern, code_buf):
                start_index = f"1.0+{match.start()}c"
                end_index = f"1.0+{match.end()}c"
            ...
            self.english_text.tag_config("string", foreground="#FF0000")
             ...
             # Highlight comments (gray) in English pane
            for match in re.finditer(comment_pattern, english_buf):
                start_index = f"1.0+{match.start()}c"
                end_index = f"1.0+{match.end()}c"
                self.english_text.config(state="normal")
   ...
   # ---------- Theme Toggle ----------
    ...
    def increase_font_size(self):
        ...
        self.code_text.config(
            font=("Courier New", self.editor_font_size)
        )
        ...
        def check_queue():
            try:
                while True:
                    ...
                    self.result_text.tag_config("done_msg", foreground="#FF0000") # <-- New Tag
