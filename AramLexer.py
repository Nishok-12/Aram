...
KEYWORDS = {u'காட்டு': Token('PRINT'),
            ...
            u'பொய்': Token('FALSE'),
            u'எனில்': Token('IF'),
            u'பிறஎனில்': Token('ELIF'),
            u'பொய்எனில்': Token('ELSE'),
            u'முடி': Token('END'),
            u'சுற்று': Token('FOR'),
...
class Lexer:
        ...
        while self.currentChar != None and self.hasErrored == False:
            if self.currentChar in ' \n\t':
                ...
                if self.currentChar == '=':
                    ...
                    self.tokens.append(Token('LSHIFT'))
                else:
                ...
                if self.currentChar == '=':
                    ...
                    self.tokens.append(Token('RSHIFT'))
                else:
                    self.tokens.append(Token('GREATER'))
                    self.prevChar()
            elif self.currentChar == '~':
                self.handle_number()
                self.tokens.append(Token('BIT_NOT'))
            elif self.currentChar == ',':
                self.handle_number()
    ...
    def handle_operator(self, op):
        if op == '+':
            ...
            self.tokens.append(Token('BIT_AND'))
        elif op == '|':
            self.tokens.append(Token('BIT_OR'))
        elif op == '^':
            self.tokens.append(Token('BIT_XOR'))
