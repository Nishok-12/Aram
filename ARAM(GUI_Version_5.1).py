...
#Aram IDE GUI_Version_5.1 (Supports All Highliting(Including grey Highlighting for comment lines), Input and Error Handling(exit() function unlocked) with corrected output order)
...
class ARAMWindow:
    ...
    def _setup_gui(self):
        ...
        def update_editor(event=None):
            ...
            self.english_text.tag_remove("comment", "1.0", tk.END) # New line for English pane comments
            ...
            for match in re.finditer(r'^\s*#.*$', english_code, re.MULTILINE):
                ...
                self.english_text.config(state="disabled")
            self.english_text.tag_config("comment", foreground="grey") # New line to configure comment color for English pane
